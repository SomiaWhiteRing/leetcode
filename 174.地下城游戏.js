...
const calculateMinimumHP = function (dungeon) {
  ...
  const page = []
  for (let i = 0; i < n + 1; i++) {
    page[i] = new Array(m + 1).fill(Infinity)
  }
  page[n - 1][m] = page[n][m - 1] = 1
  for (let i = n - 1; i >= 0; i--) {
    for (let k = m - 1; k >= 0; k--) {
      const hearth = Math.min(page[i][k + 1] - dungeon[i][k], page[i + 1][k] - dungeon[i][k])
      page[i][k] = Math.max(hearth, 1)
    }
  }
  console.log(page)
  return page[0][0]
